...
function downloadCsv(events: AnalyticsData['recentEvents']) {
  const header = 'id,eventType,path,visitorId,participantId,matchId,country,city,deviceType,browser,createdAt'
  const rows = events.map(e =>
    [e.id, e.eventType, e.path, e.visitorId ?? '', e.participantId ?? '', e.matchId ?? '', e.country ?? '', e.city ?? '', e.deviceType ?? '', e.browser ?? '', e.createdAt]
  ...
  a.href = url; a.download = `analytics_${new Date().toISOString().slice(0, 10)}.csv`; a.click()
...
export default function AdminAnalyticsPage() {
  ...
  if (loading) return <p className="text-slate-400 p-4">Cargando analíticas...</p>
  if (error || !data) return <p className="text-red-400 p-4">{error || 'Sin datos'}</p>
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Events by type + Device breakdown side by side */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-slate-800 rounded-xl p-5">
          <h2 className="text-white font-semibold mb-4">Eventos por tipo</h2>
              ...
              return (
                <div key={d.device} className="mb-3">
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-white">{d.count} ({pct}%)</span>
                  </div>
              ...
              return (
                <div key={b.browser} className="mb-3">
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-white">{b.count} ({pct}%)</span>
                  </div>
                  <div className="h-1.5 bg-slate-700 rounded-full">
      ...
      {countryBreakdown.length > 0 && (
        <div className="bg-slate-800 rounded-xl p-5">
          <h2 className="text-white font-semibold mb-4">Top países</h2>
      ...
      <div className="bg-slate-800 rounded-xl p-5">
        <h2 className="text-white font-semibold mb-4">Actividad reciente (últimos 50)</h2>
            ...
            <tbody>
              {recentEvents.map(e => (
                <tr key={e.id} className="border-t border-slate-700 hover:bg-slate-700/50">
                  ...
                  <td className="py-2 pr-4 text-slate-300 max-w-[160px] truncate">{e.path}</td>
                  <td className="py-2 pr-4 text-slate-400 font-mono">{e.visitorId?.slice(-8) ?? '—'}</td>
                  <td className="py-2 pr-4 text-slate-300">{[e.country, e.city].filter(Boolean).join(' / ') || '—'}</td>
                  <td className="py-2 text-slate-400">{[e.deviceType, e.browser].filter(Boolean).join(' · ') || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
