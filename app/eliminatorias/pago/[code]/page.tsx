...
type PaymentInfo = {
  ...
  fixedRate: number
  amountVes: number
  participant: { fullName: string; participationCode: string; nationalId: string; phone: string }
}
...
export default function KOPagoPage() {
  ...
  const openWhatsApp = () => {
    ...
    const monto = method === 'PAGO_MOVIL'
      ? `Bs. ${info.amountVes.toLocaleString('es-VE')} (tasa fija ${info.fixedRate} Bs/USD)`
      : `$${info.entryUsd} USD`
    const msg = [
      `Hola, ya realicé el pago de la *Quiniela Dieciseisavos 2026*.`,
      ``,
      `*Nombre:* ${p.fullName}`,
      `*Cédula:* V-${p.nationalId}`,
      `*WhatsApp:* ${p.phone}`,
      `*Método de pago:* ${method === 'PAGO_MOVIL' ? 'Pago Móvil Banesco' : 'Zelle'}`,
      `*Monto:* ${monto}`,
      form.senderBank    ? `*Banco origen:* ${form.senderBank}` : '',
      form.paymentReference ? `*Referencia:* ${form.paymentReference}` : '',
      form.paymentDate   ? `*Fecha:* ${form.paymentDate}` : '',
      ``,
      `Quedo atento/a a la verificación. Gracias.`,
    ].filter(Boolean).join('\n')
    window.open(`https://wa.me/${WA_NUMBER}?text=${encodeURIComponent(msg)}`, '_blank')
  }

  const reportViaWA = async () => {
    setReporting(true)
    setError('')
    try {
  ...
  if (success || info.payment?.paymentStatus === 'IN_REVIEW') {
    return (
      <div className="text-center space-y-4 py-8">
        <div className="text-5xl">⏳</div>
        <h1 className="text-xl font-extrabold text-amber-700">Pago reportado</h1>
        <p className="text-slate-600 text-sm">
          Tu inscripción quedará validada cuando el administrador confirme el pago.
          Mientras tanto puedes revisar tu quiniela.
        </p>
        <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3 text-sm text-green-700 text-left">
          ✅ Pago reportado · Pendiente de verificación admin
        </div>
        <button onClick={() => router.push(`/eliminatorias/mi-quiniela/${code}`)}
          className="bg-green-600 text-white font-bold py-3 px-6 rounded-xl w-full">
          Ver mi quiniela →
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="text-center">
        ...
        <h1 className="text-xl font-extrabold text-slate-800">Pago — Dieciseisavos 2026</h1>
      ...
      {method === 'PAGO_MOVIL' ? (
        <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3 space-y-1 text-sm">
          ...
          <p><span className="text-green-600">Cédula:</span> <strong>V-{KO_PAYMENT_CI}</strong></p>
          ...
          <p className="text-xs text-green-500">(Tasa fija: {info.fixedRate} Bs/USD · ${info.entryUsd} USD)</p>
        </div>
      ) : (
        <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 space-y-1 text-sm">
      ...
      {/* Paso 1: Reportar por WhatsApp */}
      <div className="bg-white border-2 border-green-200 rounded-2xl p-4 space-y-3">
        <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">Paso 1 — Realiza el pago y reporta</p>
        <p className="text-sm text-slate-600">
          Realiza el pago con los datos de arriba y luego toca <strong>Reportar por WhatsApp</strong>.
          Te abrirá WhatsApp con un mensaje listo para enviar.
        </p>
        <button
          onClick={openWhatsApp}
          className="w-full bg-green-500 hover:bg-green-600 text-white font-extrabold py-4 rounded-xl text-base shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2">
          📱 Reportar pago por WhatsApp
        </button>
      </div>

      {/* Paso 2: Marcar como reportado */}
      <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3">
        <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">Paso 2 — Confirma tu reporte</p>
        <p className="text-sm text-slate-600">
          Después de enviar el comprobante por WhatsApp, toca el botón de abajo para que el sistema registre tu pago como reportado.
        </p>
        {error && <div className="bg-red-50 border border-red-200 text-red-700 rounded-xl px-4 py-3 text-sm">{error}</div>}
        <button
          onClick={reportViaWA}
          disabled={reporting}
          className="w-full bg-amber-400 hover:bg-amber-300 text-slate-900 font-extrabold py-3.5 rounded-xl transition-all disabled:opacity-50 active:scale-95">
          {reporting ? 'Registrando...' : '✅ Ya reporté el pago'}
        </button>
      </div>

      {/* Alternativo: rellenar formulario */}
      <div className="border-t border-slate-100 pt-3">
        <button
          onClick={() => setShowForm(v => !v)}
          className="text-sm text-slate-500 underline w-full text-center">
          {showForm ? '▲ Ocultar formulario detallado' : '▼ Llenar datos del comprobante manualmente'}
        </button>
      </div>

      {showForm && (
        <form onSubmit={submitForm} className="space-y-3 bg-white border border-slate-200 rounded-2xl p-4">
          {method === 'PAGO_MOVIL' ? (
            <div>
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1">Referencia del pago</label>
            <input type="text" placeholder="Número de referencia / confirmación"
          ...
          <button type="submit" disabled={submitting}
            ...
            {submitting ? 'Enviando...' : 'Reportar pago con datos →'}
          </button>
        </form>
      )}
